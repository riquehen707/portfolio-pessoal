...
function compileLocalContent() {
  const program = ts.createProgram({
    rootNames: [
      ...
      path.join(root, "src/content/products/productSchema.ts"),
      path.join(root, "src/content/products/products.ts"),
    ],
    options: {
...
async function getArticleRecords() {
  ...
  for (const file of files.filter((entry) => entry.endsWith(".mdx"))) {
    ...
    if (data.slug) {
      const productCards = [...contents.matchAll(/<ProductCard\b([\s\S]*?)\/>/g)].map((match) =>
        Object.fromEntries([...match[1].matchAll(/([A-Za-z]+)="([^"]*)"/g)].map((attribute) => [attribute[1], attribute[2]])),
      );
      articles.push({
...
try {
  ...
  const productRoot = path.join(temporaryDirectory, "src/content/products");
  const { productCatalog } = require(path.join(productRoot, "products.js"));
  const { ProductCatalogSchema } = require(path.join(productRoot, "productSchema.js"));
  ...
  const productIds = new Set(productCatalog.products.map((item) => item.id));
  const productVariantIds = new Set(productCatalog.variants.map((item) => item.id));
  ...
  invalidRelationships.push(
    ...
    ...productCatalog.products.flatMap((product) => [
      ...(!organizationIds.has(product.manufacturerId) ? [{ type:"missing_organization", from:product.id, to:product.manufacturerId }] : []),
      ...product.relatedArticleSlugs.filter((slug) => !articleSlugSet.has(slug)).map((slug) => ({ type:"missing_article", from:product.id, to:slug })),
    ]),
    ...productCatalog.variants.filter((variant) => !productIds.has(variant.productId)).map((variant) => ({ type:"missing_product", from:variant.id, to:variant.productId })),
    ...productCatalog.offers.filter((offer) => !productVariantIds.has(offer.variantId)).map((offer) => ({ type:"missing_product_variant", from:offer.id, to:offer.variantId })),
    ...articles.flatMap((article) => article.productCards.flatMap((card) => {
      const required = ["productId", "whyIncluded", "bestFor", "mainDifference", "tradeOff", "sensiblePriceRange", "avoidWhen", "closestCompetitor"];
      return [
        ...(!productIds.has(card.productId) ? [{ type:"missing_product", from:article.slug, to:card.productId ?? "undefined" }] : []),
        ...required.filter((field) => !card[field]).map((field) => ({ type:"incomplete_product_recommendation", from:article.slug, to:field })),
      ];
    })),
  );
  ...
  const audit = {
    ...
    counts: {
      ...
      productVariants: productCatalog.variants.length,
      productOffers: productCatalog.offers.length,
    ...
    selfTests: {
      ...
      productCatalogAccepted: ProductCatalogSchema.safeParse(productCatalog).success,
