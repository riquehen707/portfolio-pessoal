...
export const movieCurations: MovieList[] = [{
  ...
  id:"list_end_of_world_movies",slug:"filmes-sobre-o-fim-do-mundo",title:"23 filmes sobre o fim do mundo que realmente valem a pena",href:"/blog/filmes-sobre-o-fim-do-mundo",mode:"editorial",
  items:[
    ["mov_end_dr_strangelove","Expõe como ego, protocolo e automatismo podem produzir o fim nuclear antes que alguém consiga assumir responsabilidade."],
    ["mov_end_threads","Recusa o cogumelo atômico como clímax e acompanha a destruição social que continua por anos."],
    ["mov_end_when_wind_blows","Reduz a guerra nuclear a um casal, uma casa e instruções públicas incapazes de protegê-los."],
    ["mov_end_sacrifice_1986","Transforma a ameaça nuclear numa crise espiritual sobre fé, promessa e o preço de salvar o mundo."],
    ["mov_end_war_worlds_1953","Converte a invasão marciana em espetáculo clássico sem perder a sensação de impotência humana."],
    ["mov_end_body_snatchers_1978","Imagina o apocalipse como assimilação silenciosa: o mundo termina quando ninguém mais consegue confiar no outro."],
    ["mov_end_worlds_end","Usa robôs e uma invasão para perguntar se sobreviver significa aceitar uma vida perfeitamente padronizada."],
    ["mov_d12a77","Faz a pandemia acelerar corpos e instituições, mas encontra a ameaça mais duradoura na violência organizada pelos sobreviventes."],
    ["mov_end_contagion","Mostra a pandemia como rede de contatos, decisões públicas, pesquisa e desinformação, sem depender de um herói único."],
    ["mov_46ed93","Comprime epidemia, conflito de classe e solidariedade num trem em que cada porta muda o problema coletivo."],
    ["mov_end_children_men","Enxerga o fim como esgotamento demográfico e político, contrapondo autoritarismo a uma esperança pequena e concreta."],
    ["mov_end_snowpiercer","Faz do último habitat humano uma miniatura violenta das classes e sistemas que ajudaram a destruir o planeta."],
    ["mov_end_fury_road","Mostra um mundo já acabado em que água, corpos e memória viraram recursos controlados por um tirano."],
    ["mov_end_road","Retira quase toda a mitologia do pós-apocalipse para observar o que pai e filho ainda chamam de humanidade."],
    ["mov_end_wall_e","Torna lixo, consumo e abandono ambiental compreensíveis sem abrir mão de humor, romance ou aventura familiar."],
    ["mov_end_take_shelter","Mantém a catástrofe entre profecia e crise psíquica, fazendo o medo destruir a segurança antes da tempestade chegar."],
    ["mov_end_melancholia","Liga a colisão de um planeta à depressão sem sugerir que uma coisa seja apenas metáfora da outra."],
    ["mov_end_aniara","Troca a explosão instantânea por décadas de deriva espacial, erosão de sentido e adaptação ao desastre irreversível."],
    ["mov_end_quiet_earth","Transforma uma Terra vazia num laboratório de solidão, culpa científica e relações frágeis entre sobreviventes."],
    ["mov_end_last_night_1998","Observa pessoas comuns escolhendo companhia, rotina ou ruptura quando o fim é público, certo e inexplicado."],
    ["mov_end_final_hours","Usa as últimas horas para contrastar festa, violência e a decisão simples de cuidar de uma criança."],
    ["mov_end_seeking_friend","Procura afeto e reparação tardia dentro de uma comédia romântica que não pode prometer futuro ao casal."],
    ["mov_end_rapture_1991","Leva o apocalipse cristão a sério o bastante para confrontar fé, julgamento e a possibilidade de recusar a salvação."],
  ].map(([movieId,context])=>({movieId:String(movieId),context:String(context)})),
}, {
  id:"list_best_batman_animated_movies",slug:"melhores-filmes-animados-do-batman",title:"Os 23 melhores filmes animados do Batman, do bom ao essencial",href:"/blog/melhores-filmes-animados-do-batman",mode:"editorial",
  items:[
    ["mov_bat_hush",23,"Preserva o romance entre Bruce e Selina e boa parte do elenco dos quadrinhos, mas a solução alterada enfraquece o mistério."],
    ["mov_bat_batwoman_mystery",22,"Mantém o acabamento do universo animado clássico e um mistério funcional, mesmo sem alcançar seus longas decisivos."],
    ["mov_bat_son",21,"Apresenta Damian Wayne com ação eficiente, embora comprima a estranheza e a escala da fase de Grant Morrison."],
    ["mov_bat_bad_blood",20,"Entende que o legado do Batman também pertence à família criada ao redor dele e distribui bem a ação entre seus membros."],
    ["mov_bat_vs_robin",19,"Usa a Corte das Corujas para dar forma ao conflito entre controle paterno, violência e autonomia de Damian."],
    ["mov_bat_soul_dragon",18,"Troca a adaptação literal por uma homenagem a artes marciais dos anos 1970, com Batman integrado a um bom conjunto."],
    ["mov_bat_doom_gotham",17,"Leva Bruce ao horror cósmico e usa a liberdade do Elseworld para alterar Gotham, aliados e destino sem pedir desculpas."],
    ["mov_bat_gaslight",16,"Transforma a Gotham vitoriana em espaço de investigação e expande a graphic novel com um elenco melhor integrado ao mistério."],
    ["mov_bat_caped_crusaders",15,"Recupera o Batman de 1966 com afeto, ritmo e consciência cômica, sem tratar aquela versão como erro histórico."],
    ["mov_bat_ninja",14,"É narrativamente caótico, mas sua mistura de anime, teatro, robôs e Japão feudal oferece uma identidade visual incomparável."],
    ["mov_bat_tmnt",13,"Faz o crossover funcionar por contraste de personalidade, lutas legíveis e combinações de vilões que não parecem aleatórias."],
    ["mov_bat_assault_arkham",12,"Usa Batman como ameaça lateral numa ótima operação do Esquadrão Suicida dentro do universo dos jogos Arkham."],
    ["mov_bat_gotham_knight",11,"A desigualdade da antologia é compensada pela variedade de estúdios, formas e interpretações visuais do mesmo mito."],
    ["mov_bat_long_halloween_2",10,"Entrega a tragédia de Harvey Dent e a transformação criminal de Gotham, ainda que algumas mudanças reduzam a precisão do mistério."],
    ["mov_bat_long_halloween_1",9,"Dá tempo ao Batman detetive, ao crime organizado e às alianças iniciais antes de Gotham ser ocupada pelos supervilões."],
    ["mov_bat_subzero",8,"Conta uma história compacta em que Mr. Freeze continua sendo pessoa trágica, não apenas obstáculo congelante."],
    ["mov_bat_year_one",7,"Traduz com disciplina a origem urbana de Bruce e Gordon, preservando a centralidade da corrupção e do fracasso."],
    ["mov_bat_lego_movie",6,"Transforma décadas de continuidade em comédia e ainda encontra uma crítica afetuosa à solidão performada por Bruce."],
    ["mov_bat_dkr_1",5,"Faz envelhecimento, obsessão e retorno pesarem no corpo de Batman antes de ampliar o confronto político."],
    ["mov_bat_dkr_2",4,"Conclui a adaptação com Coringa, Superman e Estado empurrando a visão de Frank Miller até sua escala mais operística."],
    ["mov_bat_beyond_joker",3,"Confronta Terry e Bruce com o trauma de Tim Drake e oferece ao Coringa um retorno ligado diretamente ao legado do Batman."],
    ["mov_bat_red_hood",2,"Converte uma continuidade complexa dos quadrinhos num conflito claro e doloroso sobre luto, vingança e o limite moral de Bruce."],
    ["mov_bat_phantasm",1,"Une investigação, romance, origem, vilão original e Kevin Conroy na síntese mais completa do Batman em animação."],
  ].map(([movieId,position,context])=>({movieId:String(movieId),position:Number(position),context:String(context)})),
}];
