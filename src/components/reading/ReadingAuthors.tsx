...
export function ReadingAuthors({ credits }: { credits: readonly Credit[] }) {
  ...
  return <div className={styles.credits}>{credits.map((credit) => {
    ...
    return <span key={`${credit.personId}-${credit.roles.join("-")}`}><strong>{credit.roles.map(readingLabel).join(", ")}:</strong> {person?.status === "published" && person.profilePath ? <Link href={person.profilePath}>{name}</Link> : name}</span>;
  })}</div>;
}
