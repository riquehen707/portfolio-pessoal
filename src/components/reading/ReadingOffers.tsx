...
export function ReadingOffers({ offers }: { offers: readonly ReadingOffer[] }) {
  ...
  return <aside className={styles.offers} aria-label="Ofertas desta edição">{offers.filter((offer) => offer.availability === "available" || offer.availability === "preorder").map((offer) => <div key={offer.id}><a href={offer.url} target="_blank" rel={offer.affiliateProgram ? "sponsored nofollow noreferrer" : "nofollow noreferrer"}>{offer.store}</a>{offer.commissionDisclosure && <small> · {offer.commissionDisclosure}</small>}</div>)}</aside>;
}
