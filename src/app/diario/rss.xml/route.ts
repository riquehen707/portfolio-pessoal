...
export async function GET() {
  ...
  const items = posts
    .map((post) => {
      ...
      const link = `${baseURL}${daily.path}/${post.slug}`;
...
<rss version="2.0">
  <channel>
    <title>${escapeXml(daily.title)}</title>
    <link>${baseURL}${daily.path}</link>
    <description>${escapeXml(daily.description)}</description>
