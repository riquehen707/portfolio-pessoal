...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title, description: work.shortDescription, alternates: { canonical: url }, robots: { index: true, follow: true }, openGraph: { title, description: work.shortDescription, url, type: "book", images: work.image ? [{ url: `${baseURL}${work.image.src}`, alt: work.image.alt }] : undefined } };
}
...
export default async function BookPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const preferredEdition = publishedEditions.find((edition) => edition.country === "Brasil" && edition.cover) ?? publishedEditions.find((edition) => edition.cover);
  ...
  return <main className={`${styles.page} ${styles.detailPage}`}>
    ...
    <article className={styles.bookProfile}>
      ...
      <header className={styles.bookHero}>
        <div className={styles.badges}><span>{readingLabel(work.format)}</span><span>{readingLabel(work.publicationStatus)}</span>{work.publicationStart ? <span>{work.publicationStart}</span> : null}</div>
    ...
    <div className={styles.contentLayout}>
      <div className={styles.mainColumn}>
        {work.audienceProfile || work.themes.length ? <section className={styles.editorialSection} aria-labelledby="sobre-a-obra"><span className={styles.kicker}>Visão geral</span><h2 id="sobre-a-obra">{work.audienceProfile ? "Sobre a obra" : "Temas e recortes"}</h2>{work.audienceProfile ? <p>{work.audienceProfile}</p> : null}{work.themes.length ? <div className={styles.tags} aria-label="Temas da obra">{work.themes.map((theme) => <span key={theme}>{theme}</span>)}</div> : null}</section> : null}
