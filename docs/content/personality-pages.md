...
Cada registro mantém ID permanente, slug, nome, nome completo ou original quando necessário, estado editorial, datas e local de nascimento e falecimento, país ou região, ocupações, apresentação, biografia breve, temas, ideias principais opcionais, pontos de entrada, personalidades e links relacionados, fontes e datas de manutenção. `birthYear` e `deathYear` atendem datas aproximadas ou anteriores à era comum, que não cabem no formato ISO de `birthDate` e `deathDate`.
...
2. Resumo rápido: temas, período e interlocutores suficientes para orientar a navegação.
...
4. Por onde começar: pequena trilha de referências por ID, cada uma com justificativa editorial curta; a ordem deve ser apresentada como sugestão, não hierarquia absoluta.
5. Obras no acervo: relações reversas, agrupadas por tipo. Bibliografias extensas usam o carrossel genérico com HTML e links renderizados no servidor, scroll horizontal natural e controles progressivos sem autoplay.
6. Trajetória e contexto: dois ou poucos parágrafos verificáveis e temas relevantes, depois das orientações de leitura e das obras.
7. Conteúdos e personalidades relacionados: links internos confirmados e pessoas referenciadas por ID. Perfis ainda em rascunho aparecem apenas como contexto, sem link para rota pública inexistente.
8. Fontes e créditos: referências biográficas e licença da imagem visíveis.

## SEO e publicação

Perfis públicos precisam de `status: "published"`, biografia suficiente e ao menos uma fonte confiável. A rota gera título, descrição, canonical, Open Graph, breadcrumbs e JSON-LD `WebPage` com `mainEntity: Person` e ID semântico `#person`, apenas com dados visíveis. Perfis incompletos continuam como rascunho, fora dos parâmetros estáticos e do sitemap. Bibliografias usam somente obras publicadas. Uma filmografia factual pode projetar registros centrais ainda em revisão quando o vínculo profissional estiver confirmado e o card permanecer sem link para página individual; a apresentação deve separar funções como direção e roteiro. Registros sem crédito confirmado não aparecem.
