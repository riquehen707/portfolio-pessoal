...
Quando a solicitação aprofunda uma inspiração já publicada, a composição pode permanecer em `/servicos/inspiracoes/[slug]` para não duplicar a mesma direção em `/servicos/exemplos`. O campo opcional `publication` de `ServiceInspiration` escolhe um renderer editorial específico; a fonte de dados, o slug, o retorno à galeria e o CTA contextual continuam compartilhados. Esse aprofundamento não transforma a referência em case, template pronto ou imóvel, produto e negócio reais. `imoveis-em-destaque`, `portfolio-fotografico`, `arquitetura-editorial`, `bem-estar-acolhedor`, `negocio-local-vibrante` e `portfolio-minimalista` usam renderers próprios em `src/components/services/inspirations/`. As publicações novas possuem dados próprios e não dependem das demonstrações legadas em `/servicos/exemplos`.

Inspirações que ainda não justificam um renderer exclusivo usam a publicação editorial compartilhada em `src/components/services/inspirations/standard/`. Cada registro mantém em `journey` quatro momentos adaptados ao contexto, além da imagem, descrição, tags, referências relacionadas e CTA. Essa camada melhora orientação e conversão sem fingir interatividade específica do segmento.
